...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True):
    ...
    if len(gpu_ids) > 0:
        ...
        # if not amp:
        # net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs for non-AMP trainingnet
...
def define_F(input_nc, netF='mlp_sample', norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    if netF == 'mlp_sample':
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    # WE implement, 'PatchGAN' style Discriminator from pix2pix paper. It can classify whether 70×70 overlapping patches are real or fake.
...
class PatchSampleF(nn.Module):
    ...
    def create_mlp(self, feats):
        for mlp_id, feat in enumerate(feats):
            ...
            print(f"Creating MLP {mlp_id}: input_nc = {input_nc}, self.nc = {self.nc}")
            mlp = nn.Sequential(
    ...
    def create_kan(self, feats): # We found that (grid_size=5, spline_order=3) OR (grid_size=7, spline_order=5) results in better performance
        for kan_id, feat in enumerate(feats):
            input_nc = feat.shape[1]
            print(f"Creating KAN {kan_id}: input_nc = {input_nc}, self.nc = {self.nc}")
            kan = KAN(
                layers_hidden=[input_nc, self.nc, self.nc],
                grid_size=5,
                spline_order=3,
                scale_noise=0.1,
                scale_base=1.0,
            ...
            setattr(self, 'kan_%d' % kan_id, kan)
        init_net(self, self.init_type, self.init_gain, self.gpu_ids)
        self.mlp_init = True
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if self.use_mlp:
                ...
                kan = getattr(self, 'kan_%d' % feat_id)
                x_sample = kan(x_sample)  # Efficient KAN forward pass
